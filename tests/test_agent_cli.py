...
import anomx.agent.ui.views.matrix_loading as matrix_loading_module
...
def test_list_sessions_reuses_cached_record_for_unchanged_transcript(tmp_path, monkeypatch):
    home = AnomxHome(tmp_path / "home")
    repo = tmp_path / "repo"
    repo.mkdir()
    ...
    parse_calls = 0
    original_read_session_record = home._read_session_record

    def counting_read_session_record(path):
        nonlocal parse_calls
        parse_calls += 1
        return original_read_session_record(path)

    monkeypatch.setattr(home, "_read_session_record", counting_read_session_record)

    home.list_sessions(limit=None)
    assert parse_calls == 1

    # The transcript on disk hasn't changed, so a second listing (as happens on every
    # startup, project switch, and animation check) must hit the cache instead of
    # re-parsing the full JSONL body again.
    home.list_sessions(limit=None)
    assert parse_calls == 1
...
def test_startup_loading_exits_when_platform_heartbeat_succeeds(
    ...
    monkeypatch.setattr(ui_module.time, "monotonic", lambda: clock["now"])
    monkeypatch.setattr(ui_module.time, "sleep", fake_sleep)
    monkeypatch.setattr(matrix_loading_module, "heartbeat_platform_connection", lambda _home: True)
    window = Window()
